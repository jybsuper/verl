...
class AsyncRolloutRequest(BaseModel):
    ...
    def update_input_ids(self, new_input_ids: List[int], attention_mask: bool, loss_mask: bool, full_tokens: bool = False) -> None:
    ...
    def tokenize_all_messages(self, tokenizer: PreTrainedTokenizer, delta_input_ids_to_check: Optional[list[int]]) -> None:
        full_input_ids = tokenizer.apply_chat_template(self.messages_dumps, add_generation_prompt=True, tokenize=True)
        ...
        self.update_input_ids(full_input_ids, attention_mask=True, loss_mask=False, full_tokens=True)

    def get_prompt_ids(self, tokenizer: PreTrainedTokenizer) -> list[int]:
        if self.tokenization_mode == "fast":
            self.update_input_ids(self.generation_prompt_ids, attention_mask=True, loss_mask=False)
        else:
            self.tokenize_all_messages(tokenizer, self.generation_prompt_ids)
    ...
    def add_assistant_message(
        ...
        if self.tokenization_mode != "full":
            ...
            if self.tokenization_mode == "fast":
                self.update_input_ids(content_ids, attention_mask=True, loss_mask=True)
                return

        self.tokenize_all_messages(tokenizer, content_ids)
    ...
    def add_tool_response_messages(self, tokenizer: PreTrainedTokenizer, contents: list[str]) -> None:
        ...
        if self.tokenization_mode != "full":
            ...
            if self.tokenization_mode == "fast":
                self.update_input_ids(response_token_ids, attention_mask=True, loss_mask=False)
                return

        self.tokenize_all_messages(tokenizer, response_token_ids)
